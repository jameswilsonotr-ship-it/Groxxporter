...
if [ ! -f "$APK_PATH" ]; then
  echo "Error: No APK file found. Please build the applet first."
  exit 1
fi
...
# Copy APK into 'app/build/outputs/apk/debug/' within the zip
mkdir -p "$TEMP_DIR/app/build/outputs/apk/debug"
cp "$APK_PATH" "$TEMP_DIR/app/build/outputs/apk/debug/app-debug.apk"

# Also copy to root of the zip for legacy convenience
cp "$APK_PATH" "$TEMP_DIR/app-debug.apk"

# Also copy to '.build-outputs/app-debug.apk' inside the zip
mkdir -p "$TEMP_DIR/.build-outputs"
cp "$APK_PATH" "$TEMP_DIR/.build-outputs/app-debug.apk"
...
Generated on: $(date)
Included assets:
- app-debug.apk (compiled binary at root)
- app/build/outputs/apk/debug/app-debug.apk (native Gradle structure)
- .build-outputs/app-debug.apk (system output structure)
